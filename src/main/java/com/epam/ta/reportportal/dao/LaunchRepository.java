...
public interface LaunchRepository extends ReportPortalRepository<Launch, Long>, LaunchRepositoryCustom {
	 ...
	 * @return `true` if {@link TestItem#getLaunchId()} equal to provided `launchId`, {@link TestItem#getParent()} equal to `NULL`
	 * and {@link TestItemResults#getStatus()} is not equal to provided `status`, otherwise return `false`
	 */
